...
### 新增
- **自定义图片背景支持 MP4 动图**：图片皮肤以往只支持 GIF 动图 —— GIF 能被 Chromium 原生播放是因为它走 `background-image`，而 `<div>` 放不了视频，因此 MP4 改用一个独立的 **`<video>` 层**承载：静音、循环、自动播放。取景 / 缩放 / 不透明度 / 文字明暗与图片**完全一致**（同一套 crop+zoom 公式，只是从 `background-size/position` 换成盒几何）；导入即把文件复制进数据目录，删原文件不影响。
  - 支持格式由「PNG / JPG / GIF 动图 / WebP」扩展为「PNG / JPG / GIF 动图 / WebP / **MP4 动图**」，选图对话框同步放开 `mp4`。
  - 尺寸从 MP4 文件头的 `tkhd`（16.16 定点）读取；读不到时由 `<video>` 自带的显示尺寸兜底并回写一次。`mdat` 媒体数据按 box 长度**跳过而不读入内存**，避免导入大视频时打爆主进程内存。
  - 文件仍以原子复制方式落到 `userData/skins/`（先写 `.tmp` 再 `rename`），保留原有落盘口径。

### 已知限制（如实说明）
- **MP4 音轨一律不播放（强制静音）**：背景动图本就不应出声，`<video>` 固定带 `muted` 标签属性（Chromium 的免交互自动播放也要求 muted）。
- **无法采样视频首帧亮度**：文字明暗默认按**浅色兜底**（与 GIF 同口径），需要时可在「自选图片」窗口里手动改文字明暗 / UI 清晰度。

### 修复
- **浮动窗口连点「第二次点不出来」**（用户报告）：`toggleFromTray` 原为**裸可见性取反** —— 第一次点击唤起窗口时 `show()` 是异步的，`isVisible()` 在窗口真正显现前仍返回 `false`，于是紧随其后的第二次点击把**刚弹出的窗口又 `hide()` 掉**，表现为「第二次点没反应」。现加**连点归并**：`TOGGLE_DEBOUNCE_MS = 350`（与既有 `guardBlur(350)` 同值，不引入第二个常数），350ms 内的重复点击被合并为一次。实测连点 120/200/320ms 最终均可见、500ms 可正常关闭，单次点击立即生效。
- **自选图片 / 动图背景下「今日」色块像一坨纯色**（用户报告）：今日格原为 `linear-gradient(180deg, var(--accent-strong), var(--accent-strong))` —— 同色渐变等于**不透明实心块** + 白字，一旦背景是照片/动图便与背景彻底割裂、像贴上去的色卡。现**仅**在 `html[data-skin="image"]`（含视频）下把今日块改为半透明强调色 `color-mix(in srgb, var(--accent-strong) var(--today-fill-alpha, 80%), transparent)` + 保留 1.5px 内环：照片从背后透出、今日仍一眼可辨。**纯色背景逐字未改**（原生背景的进一步调整见下条）；填充强度由 `--today-fill-alpha` 单变量驱动（第 2 轮由 80% 调至 68%，见下条）。
- **「今日」的日期数字被状态色反压、几乎不可见**（既有缺陷）：今日格的日期数字与副字应为白字，但周末 / 节日 / 选中 / 区间 / 提醒等 **11 条状态色规则**的选择器特异性高于「今日白字」规则 —— 例如周末规则 `.cell.weekend:not(.other):not(.workday):not(.holiday) .d` 是 **6 个类 (0,6,0)**，压过今日白字 `.cell.today:not(.other) .d` 的 **(0,4,0)**。**今天恰逢周六**，今日格里的数字「12」因此被染成周末红（对比度仅 1.05）、几乎不可见。修法：给这 11 条状态色规则统一加今日守卫 **`:where(:not(.today))`**（`:where()` 特异性贡献为 0 ⇒ 改后特异性**逐位不变**、非今日格**零行为变化**），今日格颜色遂由 today 规则独占。**刻意不用裸 `:not(.today)`** —— 裸 `:not()` 会给选择器**加 1 个类的权重**，反而让它新赢过原先输给的规则（例：`.cell .sub.sub-festival` 会反超 `.cell.other .d/.sub`，把其它月份的节日格从淡色变成绿字）。**此缺陷 v3.3.0 出厂版同样中招，本版一并修复。**
- **默认（native）面板不再半透明**（用户可见修复）：native 皮肤主窗的中性纸面 `--paper` 由半透明 `rgba(…)` 改为**完全不透明** `rgb(…)`（用户选定完全不透明；四档 token：`:root` 白 `rgb(252, 251, 249)` / 夜 `rgb(28, 32, 44)`、`[data-theme="light"][data-style="default"]` 与 `[data-theme="dark"][data-style="default"]` 同值）。**仅 native 态生效** —— `--paper` 全仓唯一消费者是 `#widget{background:var(--paper)}`，而 `[data-skin="color"]/[data-skin="image"]` 已分别改写 `#widget` 背景，故图片 / 动图 / 纯色背景**逐字不受影响**；刻意材质档（minimal/glass/neu/tech/warm）一律不动；浮窗卡面（`dock.html`）仍为 0.95，本次只改主窗。
- **「今日」格更透明 + 加光影，并扩展到原生皮肤**（用户要求）：用户原话「今日的颜色再透明一点点，加一点点光影特效，能更突出今天，又不会影响周边单元格，也不会严重影响背景图片，这个设置仅限于原生皮肤默认和自选图片模式」。做法：① 把原**仅** `html[data-skin="image"]` 才有的「半透明填充」**扩展到原生皮肤档** `html:not([data-skin])`（与 image 档**同款**规则）；② `--today-fill-alpha` 由 **80% → 68%**（更透一档，仍为单变量驱动，不写死）；③ 加「光影」时把原来会溅到相邻格的 `0 2px 10px` **外发光**换成**纯 inset（主题色内描边 + 顶部内高光）** + 仅 **1px / 0-blur 外光圈**（外溢 ≤1px，落在 2px 格间距内 ⇒ 不再影响周边单元格）。**纯色档 `[data-skin="color"]` 逐字不动**（用户明确「仅限原生 + 自选图片」，`html:not([data-skin])` 天然不命中它）；`neu`（新拟态）材质档保留其拟态内阴影。今日 `.d`/`.sub` 仍为白字。
- **调皮肤参数时日历主窗消失，看不到实时效果**（用户报告）：`win.on('blur')` 的兄弟窗豁免名单（逐个 `isFocused()` 判定）**漏掉「自选图片」窗 `skinCustomWin`** —— 用户正是在这里调 MP4 的取景 / 缩放 / 不透明度，该窗一获得焦点就无人豁免 → 落到 `hideMain()` 把日历收走。现补上 `skinCustomWin` 守卫；并在 `openSkinCustomWindow()` 首行加 `guardBlur(350)`（与 `toggleFromTray` 同值），盖住 `show()/focus()` 的异步竞态 —— 新窗真正拿到焦点前主窗的 `blur` 可能先到，单靠 `isFocused()` 仍会漏一帧。
- **设了 MP4 动图却「回默认皮肤」/ 看不到动图**（用户报告）：根因两层 ——（a）**身份丢失**：「自选图片」窗回写取景 / 透明度时只带 `{file,crop,zoom}` / `{file,opacity}`、**不带 `kind`**，主进程 `applySkinSet` 的 image 分支虽有 `prev.kind` 兜底，但**旧档**（盘上配置无 `kind` 字段）会退化成默认 → 渲染层拿 `<img>` 去加载 `.mp4` ⇒ 背景空白；现两处回写**一律带上 `kind`**，且 `normalizeImageSpec` 对**缺失** `kind` 的存量配置按已清洗文件名的扩展名自证 —— `.mp4 → 'video'`、其余 `→ 'image'`（显式 `kind` 仍优先、脏值如 `'VIDEO'` 不猜），用户盘上 3 个无 `kind` 的 `calendar_*.mp4` 因此直接救回。（b）**背景来源被整体写回原生**：全仓**唯一**会**同时**把四个界面写成 `bg='native' + image=null` 的入口是 `applyNativeStyleAll()`（点「原生皮肤」某个风格行触发）；单面写回另有两处（取消跟随的 `materializeFromCalendar`、`applySkinSet` 的 `field==='bg'` 清空分支）。为让下次复现可直接定位，`applySkinSet` 末尾新增**一行诊断日志**（仅 `field ∈ {image,bg,style}` 时打 `skin: surface=… field=… bg=… kind=… file=…`，落进 `calendar.log`）。
- **日历界面 MP4 会变透明**（用户报告）：用户原话「目前只发现**日历界面** MP4 会变透明……把**桌面插件**设为 MP4……**没有变得不透明**」。根因：`template.html` 的 `[data-skin="image"] #widget { background: transparent; }`，而媒体层（`#skinImg` / `#skinVidWrap`）只有**成功铺图 / 起播后**才有内容 —— 媒体失败 / 未就绪时整窗「什么都看不到」= 透明；浮窗 / 桌面插件自带不透明卡面（`dock.html` 的 `--card`），故只有主窗表现为透明。修法：`app.js` 维护 `documentElement.dataset.mediaBlank`（无图 / 视频未 `loadeddata` / `error` / 自动播放被拒 / 图片探针失败 → `'1'`；视频 `loadeddata` 成功 / 图片探针成功 → `'0'`），`template.html` 增一条**属性门**规则 `html[data-skin="image"][data-media-blank="1"] #widget { background: var(--paper); }`。**媒体正常时 `data-media-blank='0'`，原 `:671` transparent 规则逐字生效 ⇒ 观感零变化**；`--paper` 是全主题已定义的中性纸面 token，兜底不引入新颜色口径。
- **MP4「播一下就停、再也不动」**（用户报告）：用户原话「不知道是播放结束还是我的操作导致 MP4 不动了」。根因：`skin://` 协议 handler 用 `net.fetch(pathToFileURL(file))` 且**不转发入站 `Range` 头**，响应恒为 `200`、无 `Content-Range`/`Accept-Ranges` ⇒ **status / 响应头不合格，媒体据此判定不可 seek**；用户这个 MP4 **没做 faststart（`moov` 在文件末尾）**，Chromium 的 MP4 demuxer 必须回尾部读到 `moov` 才能起播 / `loop` 回跳 —— 不可 seek 就永远取不到 `moov` ⇒ 表现为「播一下就停住」。**三组实测对照**（QA 独立最小实验，原始证据 `.tmp-diag/qa-mp4/taskA-{A,B,C}.json`）：① 不转发 `Range`（原路径）→ `status=200`、无 `Content-Range`/`Accept-Ranges`、**响应体是整文件 29889877 字节**，`<video>` seekable=`[[0,0]]`、seek 到 12s 失败（`currentTime` 停在 0）；② **把入站 `Range` 转发给 `net.fetch(file://…)`** → `status` **仍=200**、**仍无 `Content-Range`/`Accept-Ranges`**，但**响应体已跟着请求切到 1024 字节**（「体切了、头不合格」），`<video>` seekable 仍=`[[0,0]]`、seek 仍失败 ⇒ 决定可不可 seek 的是 **status / 响应头**、不是响应体是否被切，故「转发 `Range`」无效；③ **自实现 206** → `status=206` + `Content-Range: bytes 0-1023/29889877` + `Accept-Ranges: bytes`，`<video>` seekable=`[[0,24.45]]`、seek 到 12s 成功（`currentTime=12`）。**最硬的因果链**：改前 `<video>` **只会发 `range: bytes=0-`、从不回尾部取 `moov`**；自造 206 之后才出现 `bytes=29851648-` / `bytes=17498112-` / `bytes=11665408-` / `bytes=1048576-` 这类**回尾部取 `moov`** 的请求。**真机因果（QA 秒级实测）**：改前日历界面 MP4 `seekable=[[0,0]]`、`currentTime` 推进到 `ct=21.082` 冻结、`paused=true`、`error.code=3`（`PIPELINE_ERROR_DECODE`）、`buffered=[[0,23.367]]`；改后 `seekable=[[0,24.451]]`、`ct` 正常回环（23.152→24.165→0.646→…→15.876）、`error=null`。修法：`skin://` handler **自实现 206**（`skinFileResponse`：解析 `bytes=s-e`，`fs` 读切片，回 `206 + Content-Range + Accept-Ranges: bytes + Content-Length + Content-Type`；无 `Range` → 200 全量 + `Accept-Ranges`；越界 → 416）；`skinUrlToName` 的路径穿越清洗与原 404 行为**逐字保留**。另补**媒体诊断留痕**：`<video>` 的 `onerror / onstalled / onended` 与 `play()` 被拒，经 `preload` 的 `mediaDiag → ipcMain.on('media-diag')` 写进 `calendar.log` —— 此前这些事件被静默吞掉，「MP4 不动」零证据。

### 测试
- `tests/qa-v340.js` 扩至 **126 条断言**，覆盖十类：
  - **MP4 动图**：`readMp4Size` 合成 buffer 全覆盖（v0/v1、无 moov、宽高为 0、截断、**mdat 在前仍能找到 moov**、音频轨在前）、`normalizeImageSpec` 的 `kind` 分派、`importSkinImage` 的 MP4 全链路（真复制进临时目录 + 反向对照 PNG/GIF 未被误伤）、渲染层与构建产物的文本断言、以及 `toPNG()`/`_frame.png` 仍零命中的负向断言。
  - **§6 `importSkinImage → applySkinSet` 端到端回归**（补上原盲区）：对话框回值经 `applySkinSet` 显式字段重建后 `kind` 仍为 `'video'`；渲染层无 `kind` 回写继承 `prev.kind`；普通图片仍 `'image'`；脏 `file` 清洗为 `null` 时既有视频引用原样保留。
  - **§7 连点归并**（本版「连点」修复）：`toggleFromTray` 重入窗口的时间可控双点击 —— 连点最终可见、窗口外正常 hide、边界「<窗口忽略 / =窗口放行」、单次点击立即生效；连点数值从源码 `TOGGLE_DEBOUNCE_MS` 读取、不写死。
  - **§8 `package.json` 两串自检**：`check` 与 `prebuild` 逐字相同且都含 `node tests/qa-v340.js`。
  - **§9「今日恒白字」扫雷**（本版「今日白字」修复）：读**重建后的 `calendar.html`**，断言「不存在能压过今日白字的 `.d`/`.sub` 颜色规则」（解析器跨行选择器归并自测 + 锚点唯一 + 系统性扫雷 + `:where` 守卫特异性不变），并含 **3 条反向自测**（删守卫 / 注入 (0,7,0) 无守卫规则 → 必判红；注入带守卫的红字 → 不得判红）证明闸门真会红。
  - **§10「中性纸面完全不透明」锁**（本版「默认面板不透明」修复）：断言四个中性档 `--paper` 为**无 alpha 的 `rgb()` 绝对口径**（`rgb()` 无第 4 段、或 `alpha==1`）、刻意材质档不越界（glass ≤0.42、8 个 hex 原值）、**产物 `calendar.html` 4 处同步为无 alpha**；并含 **2 条反向自测**（注入 `0.95` / 注入 `0.88` 均必须判红 —— 锁定的是「完全不透明」而非「比 0.95 更实」这类相对口径）。
  - **§11「今日格半透明 + 光影」锁**（本版「今日更透明」修复）：`--today-fill-alpha` **等值**断言 `68%`（绝对口径，非阈值）；断言**原生档与 image 档都**走 `color-mix` 半透明填充 + 顶部内高光；断言**纯色档未被任何 `color-mix` 今日规则命中**；并含 **1 条反向自测**（把 `68%` 改回 `80%` 必判红）与「今日 `.d`/`.sub` 仍 `#fff`」不变量。
  - **§12「自选图片窗 blur 豁免」锁**（本版「调皮肤时日历消失」修复）：抠 `win.on('blur')` **代码体**后逐窗断言 `dockWin/desktopWin/settingsWin/skinWin/skinCustomWin/remindlistWin/reminderWin` 均有 `isFocused()` 守卫；并含 **1 条反向自测**（删掉 `skinCustomWin` 守卫后扫描必判红）+ `openSkinCustomWindow()` 体内含 `guardBlur(350)`。
  - **§13「MP4 身份随行 + 存量扩展名自证」锁**（本版「设了 MP4 回默认皮肤」修复）：`normalizeImageSpec` 对无 `kind` 的 `.mp4` 推断为 `'video'`、`.png` 为 `'image'`（含大小写不敏感、显式 `kind` 优先、非法值不猜扩展名）；渲染层只回写 `{file,opacity}` / `{file,crop,zoom}` 时 `kind` 不回退、`bg` 不被写回 `native`、`image` 不被置 `null`；`applySkinSet` 对 `image/bg/style` 打诊断日志而 `clarity` 不打；源码锁 `skincustom.html` 的 `kind` 在 `setField()` **唯一收口点**补齐、且取景 / 透明度两处调用点仍是**原字面量**（不许为过闸门而放松 `verify_v1721.js`）。
  - **§14「媒体层空白底衬 + `skin://` Range」锁**（本版「MP4 变透明 / 播一下就不动」修复）：断言底衬规则 `html[data-skin="image"][data-media-blank="1"] #widget` 存在且 `background=var(--paper)`（非 transparent）；**★逐字锁** `[data-skin="image"] #widget { background: transparent; }` 仍在（媒体正常时观感零变化）；`app.js` 三处置 `'1'`（视频 `onerror` / `readyState<2` / 图片探针失败）两处置 `'0'`（`loadeddata` / 探针成功）；`skin://` 自实现 `206 + Content-Range + Accept-Ranges + Content-Type` 且保留 `skinUrlToName` 清洗与 404；并含 **1 条反向自测**（删掉 `Accept-Ranges` 必判红）。
- `npm run check` / `prebuild` 链尾追加 `tests/qa-v340.js`。
...
### 新增
- **浮窗样式（六款造型）**：浮窗不再只有「贴任务栏的小横条」一种形态，可在**皮肤设置 → 浮窗样式**里六选一：
  | 造型 | 名称 | 卡片尺寸 | 特征 |
  |---|---|---|---|
  | `rect` | 圆角卡片 | 114×(任务栏高-2) | **默认**，完全跟随皮肤（原生六风格 / 自选图片），跟随主界面时即为它 |
  | `pixel` | 像素方屏 | 156×64 | 珍珠白机身 + 黑屏，**3×5 真点阵块**拼出的蓝光数字 |
  | `rainbow` | 虹彩流光 | 116×44 | 一层近乎透明的玻璃薄片，只留彩虹渐变内容，无实边框 |
  | `ring` | 极简圆环 | 150×150 | 磨砂玻璃圆盘 + 圆环进度（随秒走满一圈） |
  | `flip` | 翻页时牌 | 196×104 | 三块深色翻牌 时/分/秒（**24 小时制，无午别**）+ 上排日期星期 |
  | `toon` | 绘本台钟 | 158×174 | 简笔画 / 儿童绘本风：粗暖墨线 + 蜡笔平涂（色块故意涂出界）+ 手写体数字 + 三针走真实时间 |
- **皮肤设置主窗口新增第三个入口「浮窗样式」**：与「原生皮肤」同构 —— 点击动画展开 6 行，每行左侧一枚**等比缩放的造型缩略图**（轮廓 + 主色 + 材质暗示），「圆角卡片」行标注「跟随主界面」。
- **「自选图片」窗口新增「清空」按钮**：一键清掉当前界面的自选图片、回到原生皮肤。按钮仅在该界面**自己有图片**时可用。盘上图片文件不删除。
- **「浮窗大小」等比缩放拉杆**：在「自选图片」窗口选择「浮窗界面」时，新增「浮窗大小 → 缩放比例」拉杆，可把整个浮窗**等比放大 / 缩小**（**60% ~ 160%，步进 5%，默认 100%**）。缩放调的是**几何尺寸**，与「跟随主界面」开关**相互独立** —— 拖动它不会让浮窗脱离跟随。

### 变更
- **浮窗尺寸随造型变化，不再被锁死在 116×任务栏高**：`DOCK_W/DOCK_H` 常数仍在，但实际意图尺寸改由造型注册表 `DOCK_SHAPES` 给出（窗口尺寸 + 四边内边距），并新增 `syncDockGeometry()` 在「造型变化 / 跟随状态变化 / 创建窗口」三处同步窗口几何。除「虹彩流光」外，非圆角造型一律 12px 四周对称内边距，以便留出投影空间（虹彩流光为 24/24/0/0，见下条）。
- **非「圆角卡片」的造型成为桌面挂件**：它们自带材质（黑屏 / 彩虹 / 玻璃 / 深色翻牌 / 台钟），因此**皮肤与风格不再作用于它们**；默认落点也从「压任务栏上沿」改为「工作区右下内缩 24px」，可拖到桌面任意位置（「虹彩流光」除外 —— 它已回归贴边形态，与「圆角卡片」走同一条落点公式，见下条）。
- **「虹彩流光」造型回归贴边形态**：它现在与默认的「圆角卡片」一样，可以**贴住任务栏上沿、贴住屏幕右沿** —— 做法是把它窗口的四边内边距由 `12/12/12/12` 改为 **`24/24/0/0`**（左 24 / 上 24 / 右 0 / 下 0，**卡片尺寸仍是 116×44 不变**），右、下内边距归零后卡片的右 / 下边缘贴死窗口边缘；**默认落点也改为与「圆角卡片」同一条公式**（贴工作区右沿 + 压任务栏上沿），不再是右下内缩 24px 的悬浮挂件。其余五款造型行为一字不变。
- **在非「圆角卡片」造型下导入自选图片，浮窗会自动切回「圆角卡片」**：像素方屏 / 虹彩流光 / 极简圆环 / 翻页时牌 / 绘本台钟这些造型自带材质、不吃图片皮肤，而「导入图片」是明确意图，故让造型让步 —— 导入后浮窗自动变回 `rect` 并显示该图片。**反向不成立**：单纯换造型不会清掉已选图片，图片留在配置里，切回「圆角卡片」即恢复显示；「清空」按钮只清图片、**不改造型**；盘上 `userData/skins/*` 图片文件一律不删。
- **取消跟随 = 回到原生皮肤**（见「修复」）：跟随开关的提示语同步改写为「开启时与主界面完全一致；单独调整任一项即自动独立，并回到原生皮肤」。
- **浮窗点击热区按造型收敛**：圆形造型（极简圆环 / 绘本台钟）改用圆形判定，点圆外面的角不再误弹日历。

### 修复
- **窗口圆角之外露出一圈不透明的直角底色**（用户报告）：原生皮肤下，日历窗口 / 桌面插件 / 放大窗口的**整个窗口矩形**被 `body` 的一层不透明渐变底色铺满，`#widget` 圆角之外因此露出四个直角色块。现去掉这层底色（**仅 Electron 端生效**，浏览器预览的卡片外留白不受影响）。
  - **顺带的正面收益**：卡片本来就是「半透明纸 + 毛玻璃模糊」（`backdrop-filter`），但改前它背后垫的是一块**不透明死板**，模糊等于没生效；改后背后是真实桌面壁纸 ⇒ **毛玻璃第一次真正生效**。
- **极简圆环（及像素方屏）造型外围明显的方形暗框**（用户报告）：这两款造型的外投影超出造型自身的垫边（12px），被窗口矩形**硬裁一刀**、残留在四角成框。现 **极简圆环的外投影整层删除**（只保留两道内侧白色高光描边，圆盘靠 20% 白描边定界）；**像素方屏的投影收进垫边之内**（`offset + blur` 不超过 12px），保留机身立体感、不再被裁出直角边。其余造型未改动：虹彩流光 / 翻页时牌 / 绘本台钟本就无越界外投影；圆角卡片贴任务栏边角的既有表现属刻意保留。
- **「自选图片里选择不跟主界面，图片框里还是主界面的图」**：本轮用户报告。根因是 `applySkinSet` 的「手调即豁免跟随」物化函数 `materializeFromCalendar()` 会把日历的 `bg` 与 `image` **一并复制**给取消跟随的界面，而图片框读的是本面自己的配置 —— 于是取消跟随反而"继承"了主界面的图。现改为**不继承图片与背景来源**（`bg='native'`、`image=null`），其余 `style/text/clarity/tone` 仍照旧继承以保持外观连续；`shape` 亦不复制（造型是浮窗自己的身份，复制会弄丢用户选的造型）。
- **`loadSettings` 的 `dockBounds` 尺寸校验**不再硬编码 `[DOCK_W±4]×[36,60]`，改为与「当前生效造型的期望窗口尺寸」比对（±8），不符则丢弃并回落默认落点 —— 否则方/竖造型的落点会被旧校验判为非法而丢失。
- **点「自选图片」弹出的文件选择对话框被皮肤窗盖住、跑到屏幕左上角**（用户报告「浮窗设置不了自选图片、无效」的另一半根因）：「皮肤设置」窗与「自选图片」窗都是**置顶窗口**（`setAlwaysOnTop(true,'screen-saver')`），而选图对话框创建时**没有指定父窗口**，于是成了一个**无主**的顶级窗口 —— 真机实测它 `WS_EX_TOPMOST=false`、几何落在屏幕左上角 `(0,0)-(720,480)`，被两个置顶皮肤窗压在下面。现改为用 `BrowserWindow.fromWebContents(evt.sender)` **反查发起方窗口**并作为父窗口传入，对话框随即成为 owned window（Windows 语义下恒在 owner 之上），几何也贴齐发起方左上角（实测 `(470,130)`）；**取不到父窗口时仍保留不传父窗口的旧调用**，不新增任何失败面。
- **浮窗选了非「圆角卡片」造型时，看不出「图片还在、只是这个造型不显示」**（用户报告「浮窗设置不了自选图片、无效」）：这几款造型自带材质、不吃图片（见「变更」），但「自选图片」窗的取景区仍照常显示「已设置」，两边对不上。现于取景区下方新增一句提示：**「当前造型不显示图片。要让这张图生效：在「浮窗样式」里把造型改回「圆角卡片」，或在这里重新导入图片（导入会自动切回圆角卡片）。」** 判据为「浮窗界面 + 本界面确有图片 + 生效造型不是 `rect`」，造型改回 `rect` 或图片清空即自动消失。文案**不写任何具体造型名**（避免与造型注册表形成第三份口径）。

### 测试
- 新增 `tests/qa-v330.js`（造型注册表与几何、`shape` 白名单与归属、**取消跟随不继承图片**的回归护栏、`dock.html` 六造型结构、`skin.html` 第三入口、`skincustom.html` 清空按钮）。本轮增补六条永久护栏：**造型外投影收进垫边**的取值不变量（含「极简圆环零外投影」）、**原生皮肤下 Electron 的 `body` 透明**规则与 `calendar.html` 产物重建证据、**浮窗缩放的归一化 / 写入分支 / 载荷恒为基础值**、**落点记录的尺寸随缩放刷新**、**`skincustom.html` 取景区提示的回显对齐**（读 `resolved` 生效造型而非本面配置，且负向不得成为第三份造型名口径）、**选图对话框必须挂父窗口**（含「无父窗口退化分支不得删除」的负向护栏）。
- `tests/qa-v244.js` 的 `FN_NAMES` 补入 `normShape`（`applySkinSet` 新增了对它的调用，孤立 eval 名单必须同步）。
- `npm run check` / `prebuild` 链尾追加 `tests/qa-v330.js`。
